...
test("会員証表示後にポイントを背後同期し実際の付与結果を一度だけ通知する",()=>{assert.match(home,/setInterval\(sync,30_000\)/);assert.match(home,/api\/v1\/me\/points\?month=/);assert.match(route,/SMAREGI_PURCHASE_THANK_YOU:/);assert.match(route,/今回のお会計で\$\{item\.grantedPoint\}ポイントが付与されました/);assert.match(route,/INSERT OR IGNORE INTO member_notifications/)});
...
test("スマレジに同じ入館加算があれば最も近い時刻の履歴へ統合する",()=>{assert.match(route,/Math\.abs\(Date\.parse\(item\.occurredAt\)-row\.occurredAt\)<=15\*60\*1000/);assert.match(route,/\.sort\(\(a,b\)=>Math\.abs/);assert.match(route,/matched\.label="入館ポイント"/);assert.match(page,/ポイント加算/);assert.doesNotMatch(page,/ポイント処理/)});
