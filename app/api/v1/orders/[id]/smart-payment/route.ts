...
export async function POST(request:NextRequest,{params}:{params:Promise<{id:string}>}){
 ...
 let customer=await env.DB.prepare(`SELECT stripe_customer_id AS customerId FROM stripe_customers WHERE member_id=?`).bind(member.id).first<{customerId:string}>();if(!customer?.customerId){const created=await stripeRequest<StripeCustomer>("/customers","POST",new URLSearchParams({"metadata[member_id]":member.id,description:"COMPASSION WORLD member"}),`customer:${member.id}`);await env.DB.prepare(`INSERT INTO stripe_customers (member_id,stripe_customer_id,updated_at) VALUES (?,?,?) ON CONFLICT(member_id) DO UPDATE SET stripe_customer_id=excluded.stripe_customer_id,updated_at=excluded.updated_at`).bind(member.id,created.id,Date.now()).run();customer={customerId:created.id};}const runtime=env as unknown as Record<string,string|undefined>,liffId=runtime.LINE_LIFF_ID??runtime.NEXT_PUBLIC_LIFF_ID,base=liffId?`https://liff.line.me/${encodeURIComponent(liffId)}`:(runtime.APP_BASE_URL??request.nextUrl.origin);
 ...
 const session=await stripeRequest<CheckoutSession>("/checkout/sessions","POST",form,`mobile-order:${id}`);await env.DB.prepare(`UPDATE orders SET stripe_checkout_session_id=?,status='PAYMENT_PROCESSING',updated_at=? WHERE id=? AND status='PENDING_PAYMENT'`).bind(session.id,Date.now(),id).run();
