...
export async function POST(request:NextRequest){
 ...
 if(order.status==="PAYMENT_PROCESSING"){if(!lockId||!deviceId)return NextResponse.json({ok:false,error:"LOCK_NOT_OWNED"},{status:409});const lock=await env.DB.prepare(`SELECT status,device_id AS deviceId,expires_at AS expiresAt FROM order_payment_locks WHERE id=? AND order_id=?`).bind(lockId,orderId).first<{status:string;deviceId:string;expiresAt:number}>();if(!lock||lock.deviceId!==deviceId)return NextResponse.json({ok:false,error:"LOCK_NOT_OWNED"},{status:409});if(lock.status==="EXPIRED"||lock.expiresAt<=Date.now())return NextResponse.json({ok:false,error:"LOCK_EXPIRED"},{status:409});if(lock.status!=="ACTIVE")return NextResponse.json({ok:false,error:"LOCK_NOT_OWNED"},{status:409});}
