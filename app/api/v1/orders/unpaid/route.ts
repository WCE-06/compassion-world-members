...
export async function GET(request:NextRequest){
 ...
 const rows=await env.DB.prepare(`SELECT o.id,o.order_number AS orderNumber,m.member_code AS memberCode,o.status,o.payment_method AS paymentMethod,o.created_at AS createdAt,o.expires_at AS expiresAt,o.pickup_at AS pickupRequestedAt,o.total_including_tax AS totalIncludingTax,o.smaregi_transaction_id AS smaregiTransactionId FROM orders o JOIN members m ON m.id=o.member_id WHERE o.member_id=? AND o.payment_method='STORE' AND o.status IN ('WAITING_STORE_PAYMENT','PAYMENT_PROCESSING','PAYMENT_RECONCILING') AND (o.expires_at>? OR o.status='PAYMENT_RECONCILING') ORDER BY o.created_at`).bind(member.id,queriedAt).all<PosOrderRow>();
 const recentExpired=rows.results.length===0?await env.DB.prepare(`SELECT id FROM orders WHERE member_id=? AND payment_method='STORE' AND status='EXPIRED' AND expires_at>? ORDER BY expires_at DESC LIMIT 1`).bind(member.id,queriedAt-30*60*1000).first<{id:string}>():null;
 return NextResponse.json({ok:true,memberCode,queriedAt:new Date(queriedAt).toISOString(),recentOrderExpired:Boolean(recentExpired),orders:await Promise.all(rows.results.map(orderDetails))},{headers:{"Cache-Control":"private, no-store, max-age=0","CDN-Cache-Control":"no-store"}});
}
