...
export async function POST(request:NextRequest){
 ...
 const event=JSON.parse(payload) as StripeEvent;const exists=await env.DB.prepare(`SELECT status FROM stripe_webhook_events WHERE event_id=?`).bind(event.id).first<{status:string}>();if(exists?.status==="PROCESSED"||exists?.status==="IGNORED")return NextResponse.json({received:true,duplicate:true});const now=Date.now();if(exists)await env.DB.prepare(`UPDATE stripe_webhook_events SET status='RECEIVED',error_message=NULL,processed_at=NULL WHERE event_id=?`).bind(event.id).run();else await env.DB.prepare(`INSERT INTO stripe_webhook_events (event_id,event_type,status,created_at) VALUES (?,?,'RECEIVED',?)`).bind(event.id,event.type,now).run();
