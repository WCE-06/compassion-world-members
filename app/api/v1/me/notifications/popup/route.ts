...
export async function POST(request:NextRequest){
 ...
 for(let attempt=0;attempt<3;attempt++){
  const row=await env.DB.prepare(`SELECT n.id,n.event_id AS eventId,n.event_type AS eventType,n.category,n.title,n.body,n.sender,n.metadata_json AS metadataJson,n.read_at AS readAt,n.occurred_at AS occurredAt,n.created_at AS createdAt FROM member_notifications n WHERE n.member_id=? AND n.read_at IS NULL AND n.created_at>=? AND n.event_type<>'MEMBER_WELCOME' AND (n.event_type<>'ENTRY_THANK_YOU' OR ((COALESCE(json_extract(n.metadata_json,'$.pointGranted'),0)=1 OR COALESCE(json_extract(n.metadata_json,'$.alreadyGranted'),0)=1) AND date(COALESCE(n.occurred_at,n.created_at)/1000,'unixepoch','+9 hours')=date(?/1000,'unixepoch','+9 hours'))) AND NOT EXISTS (SELECT 1 FROM notification_popup_deliveries d WHERE d.notification_id=n.id) ORDER BY n.created_at DESC LIMIT 1`).bind(member.id,recent,now).first<StoredNoticeRow>();
