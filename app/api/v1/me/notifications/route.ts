...
export async function GET(request: NextRequest) {
  ...
  const [rows,profile]=await Promise.all([env.DB.prepare(`SELECT id,event_type AS eventType,category,title,body,sender,read_at AS readAt,occurred_at AS occurredAt,created_at AS createdAt
  ...
  return NextResponse.json({ notices: [...dedupeStoredNotices(rows.results).map(storedNotice),welcomeNotice(profile)] }, { headers: { "Cache-Control": "no-store" } });
}
