...
export async function GET(request: NextRequest) {
  ...
  return NextResponse.json({memberId:member.id,memberCode:member.memberCode,displayName:member.displayName,points:member.pointsBalance,rank:member.memberRank??"STANDARD",nextReservation:reservation?{facilityName:"Music Studio FEBBRAIO",startsAt:reservation.startAt,endsAt:reservation.endAt}:null,session:session?{facilityName:"Music Studio FEBBRAIO",status:session.status==="ACTIVE"?"IN_USE":session.status,paymentStatus:session.paymentStatus,startedAt:session.checkedInAt?new Date(session.checkedInAt).toISOString():undefined,scheduledEndsAt:session.scheduledEndAt||undefined,unpaidAmount:session.billingAmount}:null,activeOrder:order?{orderNumber:order.orderNumber,status:orderStatus[order.status]}:null,notices:[]});
}
