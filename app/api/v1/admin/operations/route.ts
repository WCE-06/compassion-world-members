...
export async function GET(request:NextRequest){
 ...
 try{
  ...
  const [members,residents,ranks,orders,sessions,unpaid,stripe,failures,tasks,communications,todayOrders,todayStudio,todayMembers,activeStudio,urgentTasks,posMissing,inventory,latestWebhook]=await Promise.all([
   ...
   env.DB.prepare("SELECT COUNT(*) AS count FROM orders WHERE status='PAYMENT_FAILED'").first(),
  ...
  const integrations={smaregiProducts:Boolean(runtime.SMAREGI_PRODUCT_MASTER_URL||runtime.SMAREGI_PRODUCT_CREATE_URL||runtime.SELF_REGISTER_CATALOG_URL),smaregiInventory:Boolean((runtime.SMAREGI_INVENTORY_URL??runtime.SMAREGI_SPEND_RECALC_URL)&&(runtime.SMAREGI_INVENTORY_KEY??runtime.SMAREGI_SPEND_SYNC_KEY)),smaregiSales:Boolean(runtime.SMAREGI_SALES_SUMMARY_URL&&runtime.SMAREGI_SALES_SUMMARY_KEY||runtime.SMAREGI_SPEND_RECALC_URL&&runtime.SMAREGI_SPEND_SYNC_KEY),facility:Boolean(runtime.COMMON_FACILITY_GAS_URL&&(runtime.FACILITY_STAFF_API_TOKEN||runtime.FACILITY_API_TOKEN)),stripe:Boolean(runtime.STRIPE_SECRET_KEY&&runtime.STRIPE_WEBHOOK_SECRET),kitchen:Boolean(runtime.KITCHEN_API_TOKEN),line:Boolean(runtime.LINE_CHANNEL_ACCESS_TOKEN),sns:Boolean(runtime.SNS_CONTROL_API_URL&&runtime.SNS_CONTROL_API_KEY),snsUrl:runtime.SNS_CONTROL_URL??"https://wce-06.github.io/compassion-world-sns-control/"};
  const alerts=[Number(unpaid?.count??0)>0&&{kind:"PAYMENT",level:"warning",label:`利用済み・未精算が ${Number(unpaid?.count??0)}件あります`},Number(failures?.count??0)>0&&{kind:"STRIPE",level:"info",label:`決済不成立で未確定の注文が ${Number(failures?.count??0)}件あります`},Number(urgentTasks?.count??0)>0&&{kind:"TASK",level:"danger",label:`期限超過・緊急タスクが ${Number(urgentTasks?.count??0)}件あります`},Number(posMissing?.count??0)>0&&{kind:"POS",level:"warning",label:`スマレジ取引ID未反映が ${Number(posMissing?.count??0)}件あります`},!integrations.facility&&{kind:"FACILITY",level:"info",label:"スタジオ全予約APIは接続設定待ちです"},!integrations.smaregiInventory&&{kind:"INVENTORY",level:"info",label:"スマレジ在庫APIは接続設定待ちです"}].filter(Boolean);
