...
export async function GET(request:NextRequest){
 ...
 try{
  ...
  const alerts=[Number(unpaid?.count??0)>0&&{kind:"PAYMENT",level:"warning",label:`利用済み・未精算が ${Number(unpaid?.count??0)}件あります`},Number(failures?.count??0)>0&&{kind:"STRIPE",level:"danger",label:`Stripe Webhookの再処理が必要な記録が ${Number(failures?.count??0)}件あります`},Number(urgentTasks?.count??0)>0&&{kind:"TASK",level:"danger",label:`期限超過・緊急タスクが ${Number(urgentTasks?.count??0)}件あります`},Number(posMissing?.count??0)>0&&{kind:"POS",level:"warning",label:`スマレジ取引ID未反映が ${Number(posMissing?.count??0)}件あります`},!integrations.facility&&{kind:"FACILITY",level:"info",label:"スタジオ全予約APIは接続設定待ちです"},!integrations.smaregiInventory&&{kind:"INVENTORY",level:"info",label:"スマレジ在庫APIは接続設定待ちです"}].filter(Boolean);
