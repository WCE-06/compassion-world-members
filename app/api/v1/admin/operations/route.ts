...
async function salesSummary(runtime:Record<string,string|undefined>,start:number,end:number):Promise<SalesSummary>{
 const url=runtime.SMAREGI_SALES_SUMMARY_URL,key=runtime.SMAREGI_SALES_SUMMARY_KEY;
 if(!url||!key)return{available:false,amount:0,stores:[]};
 try{
  const target=new URL(url);target.searchParams.set("from",new Date(start).toISOString());target.searchParams.set("to",new Date(end).toISOString());target.searchParams.set("scope","ALL_STORES");
  const response=await fetch(target,{headers:{Authorization:`Bearer ${key}`,"X-API-Key":key},signal:AbortSignal.timeout(6000)});
  if(!response.ok)throw new Error(`HTTP_${response.status}`);
  const body=await response.json() as Record<string,unknown>,data=(body.data&&typeof body.data==="object"?body.data:body) as Record<string,unknown>,rawStores=Array.isArray(data.stores)?data.stores:[];
...
export async function GET(request:NextRequest){
 ...
 try{
  ...
  const [members,residents,ranks,orders,sessions,unpaid,stripe,failures,tasks,communications,todayOrders,todayStudio,todayMembers,activeStudio,urgentTasks,posMissing,inventory,latestWebhook,staleResidents,auditEvents,staleResidentRows,posMissingRows,allStoreSales]=await Promise.all([
   ...
   env.DB.prepare("SELECT COUNT(*) AS count FROM resident_subscriptions WHERE status IN ('active','trialing') AND updated_at<?").bind(now-26*60*60*1000).first<CountRow>(),
   ...
   env.DB.prepare("SELECT m.member_code AS memberCode,m.display_name AS memberName,r.status,r.current_period_end AS currentPeriodEnd,r.updated_at AS updatedAt FROM resident_subscriptions r JOIN members m ON m.id=r.member_id WHERE r.status IN ('active','trialing') AND r.updated_at<? ORDER BY r.updated_at ASC LIMIT 20").bind(now-26*60*60*1000).all(),
