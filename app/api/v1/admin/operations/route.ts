...
export async function GET(request:NextRequest){
 ...
 try{
  ...
  const [members,residents,ranks,orders,sessions,unpaid,stripe,failures,tasks,communications,todayOrders,todayStudio,todayMembers,activeStudio,urgentTasks,posMissing,inventory,latestWebhook,staleResidents,auditEvents]=await Promise.all([
  ...
  const alerts=[Number(unpaid?.count??0)>0&&{kind:"PAYMENT",level:"warning",label:`利用済み・未精算が ${Number(unpaid?.count??0)}件あります`},Number(failures?.count??0)>0&&{kind:"STRIPE",level:"danger",label:`Stripe Webhookの再処理が必要な記録が ${Number(failures?.count??0)}件あります`},Number(staleResidents?.count??0)>0&&{kind:"STRIPE",level:"warning",label:`住民契約の再確認が必要な会員が ${Number(staleResidents?.count??0)}件あります`},Number(urgentTasks?.count??0)>0&&{kind:"TASK",level:"danger",label:`期限超過・緊急タスクが ${Number(urgentTasks?.count??0)}件あります`},Number(posMissing?.count??0)>0&&{kind:"POS",level:"warning",label:`スマレジ取引ID未反映が ${Number(posMissing?.count??0)}件あります`},!integrations.facility&&{kind:"FACILITY",level:"info",label:"スタジオ全予約APIは接続設定待ちです"},!integrations.smaregiInventory&&{kind:"INVENTORY",level:"info",label:"スマレジ在庫APIは接続設定待ちです"}].filter(Boolean);
  return NextResponse.json({generatedAt:now,today:{date:day,orders:Number(todayOrders?.count??0),orderSales:Number(todayOrders?.amount??0),studioTransactions:Number(todayStudio?.count??0),studioSales:Number(todayStudio?.amount??0),newMembers:Number(todayMembers?.count??0),activeStudio:Number(activeStudio?.count??0)},alerts,auditEvents:auditEvents.results,members,residents,ranks:ranks.results,finance:{orders,sessions,unpaid,stripe,failures,posMissing},inventory,latestWebhook,tasks:tasks.results,communications,integrations:{...integrations,snsUrl,snsApiConfigured,inventoryConfigured:integrations.smaregiInventory,salesConfigured:integrations.smaregiSales,facilityStaffReservationsConfigured:integrations.facility}},{headers:{"Cache-Control":"no-store"}});
