...
export async function POST(request:NextRequest){
 ...
 if(!await verifyAdminPassword(email,current))return NextResponse.json({error:"CURRENT_PASSWORD_INVALID"},{status:401});
 if(next!==String(body?.confirmation??"")||next.length<12||next.length>128||!/[A-Za-z]/.test(next)||!/[0-9]/.test(next)||!/[!-/:-@[-`{-~]/.test(next))return NextResponse.json({error:"PASSWORD_POLICY"},{status:400});
 if(current===next)return NextResponse.json({error:"PASSWORD_REUSED"},{status:400});
 const record=await createAdminPasswordRecord(next),now=Date.now();
 ...
 return NextResponse.json({ok:true,changedAt:now});
}
