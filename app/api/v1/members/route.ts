...
import { MEMBER_RANK_TERMS_VERSION,rankPeriodFor } from "@/lib/member-rank";
...
export async function POST(request:NextRequest){
 ...
 const code=await randomMemberCode(),id=crypto.randomUUID(),now=Date.now(),eventId=crypto.randomUUID(),acceptanceId=crypto.randomUUID(),rankConsentId=crypto.randomUUID(),rankPeriod=rankPeriodFor(now,now);
 await env.DB.batch([
  ...
  env.DB.prepare("INSERT INTO member_policy_consents (id,member_id,terms_version,consent_type,source,agreed_at,user_agent) VALUES (?,?,?,'MEMBERSHIP_AND_POINTS','MEMBER_CARD',?,?)").bind(rankConsentId,id,MEMBER_RANK_TERMS_VERSION,now,(request.headers.get("user-agent")??"").slice(0,300)||null),
  env.DB.prepare("INSERT INTO member_rank_states (member_id,current_rank,current_rate_percent,rank_period_started_at,rank_period_ends_at,qualifying_spend_excluding_tax,rank_updated_at,next_review_at,membership_type,resident_plan_active,spend_source,spend_source_revision,spend_synced_at) VALUES (?,'STANDARD',1,?,?,0,?,?,'GENERAL',0,'NOT_SYNCED',NULL,NULL)").bind(id,rankPeriod.rankPeriodStartedAt,rankPeriod.rankPeriodEndsAt,now,rankPeriod.nextReviewAt),
