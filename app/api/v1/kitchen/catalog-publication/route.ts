...
export async function GET(request: NextRequest) {
  ...
  return NextResponse.json({ products: products.map((product) => ({ productCode: product.code, menuCategory: product.menuCategory, displaySequence: product.displaySequence, showOnSelfRegister: product.showOnSelfRegister, showOnMobileOrder: product.showOnMobileOrder })) }, { headers: { "Cache-Control": "no-store" } });
}
