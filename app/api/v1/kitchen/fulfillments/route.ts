...
export async function GET(request:NextRequest){
  ...
  const result=await env.DB.prepare(`SELECT f.id,f.order_id AS orderId,f.department,f.call_number AS callNumber,f.status,f.ready_at AS readyAt,f.called_at AS calledAt,f.updated_at AS updatedAt,i.product_name AS productName,i.quantity FROM order_fulfillments f JOIN order_items i ON i.order_id=f.order_id AND i.department=f.department WHERE f.department=? AND f.status IN ('ACCEPTED','COOKING','READY','CALLED') ORDER BY CASE f.status WHEN 'READY' THEN 0 WHEN 'CALLED' THEN 1 WHEN 'COOKING' THEN 2 ELSE 3 END,f.updated_at`).bind(department).all<{id:string;orderId:string;department:Department;callNumber:number;status:string;readyAt:number|null;calledAt:number|null;updatedAt:number;productName:string;quantity:number}>();
  const grouped=new Map<string,{id:string;orderId:string;department:Department;callNumber:number;status:string;readyAt:number|null;calledAt:number|null;updatedAt:number;items:{name:string;quantity:number}[]}>();
  for(const row of result.results){const current=grouped.get(row.id)??{id:row.id,orderId:row.orderId,department:row.department,callNumber:row.callNumber,status:row.status,readyAt:row.readyAt,calledAt:row.calledAt,updatedAt:row.updatedAt,items:[]};current.items.push({name:row.productName,quantity:row.quantity});grouped.set(row.id,current);}
