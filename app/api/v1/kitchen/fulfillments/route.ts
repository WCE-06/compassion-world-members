...
type Action="START"|"READY"|"CALL"|"PICKUP"|"RESTORE_CALL";
const actionRule:Record<Action,{from:string[];to:string;timeColumn?:string;clearColumn?:string}>={
  START:{from:["ACCEPTED"],to:"COOKING"},READY:{from:["ACCEPTED","COOKING"],to:"READY",timeColumn:"ready_at"},CALL:{from:["READY"],to:"CALLED",timeColumn:"called_at"},PICKUP:{from:["READY","CALLED"],to:"PICKED_UP",timeColumn:"picked_up_at"},RESTORE_CALL:{from:["PICKED_UP"],to:"CALLED",timeColumn:"called_at",clearColumn:"picked_up_at"},
};
...
export async function GET(request:NextRequest){
  ...
  const statusFilter=history?"f.status IN ('PICKED_UP','CANCELLED')":"f.status IN ('ACCEPTED','COOKING','READY','CALLED')";
  const result=await env.DB.prepare(`SELECT f.id,f.order_id AS orderId,f.department,f.call_number AS callNumber,f.status,f.ready_at AS readyAt,f.called_at AS calledAt,f.updated_at AS updatedAt,o.pickup_at AS estimatedReadyAt,i.product_name AS productName,i.quantity,i.preparation_minutes AS preparationMinutes FROM order_fulfillments f JOIN orders o ON o.id=f.order_id JOIN order_items i ON i.order_id=f.order_id AND i.department=f.department WHERE f.department=? AND ${statusFilter} ORDER BY f.updated_at DESC LIMIT 100`).bind(department).all<{id:string;orderId:string;department:Department;callNumber:number;status:string;readyAt:number|null;calledAt:number|null;updatedAt:number;estimatedReadyAt:number|null;productName:string;quantity:number;preparationMinutes:number}>();
...
export async function PATCH(request:NextRequest){
  ...
  const timeUpdate=rule.timeColumn?`,${rule.timeColumn}=?`:"",clearUpdate=rule.clearColumn?`,${rule.clearColumn}=NULL`:"";
  const bindings=rule.timeColumn?[rule.to,now,now,fulfillmentId,...rule.from]:[rule.to,now,fulfillmentId,...rule.from];
  const result=await env.DB.prepare(`UPDATE order_fulfillments SET status=?,updated_at=?${timeUpdate}${clearUpdate} WHERE id=? AND status IN (${placeholders})`).bind(...bindings).run();
  if((result.meta.changes??0)===0){const existing=await env.DB.prepare(`SELECT department,call_number AS callNumber,status FROM order_fulfillments WHERE id=?`).bind(fulfillmentId).first<{department:Department;callNumber:number;status:string}>();if(!existing)return NextResponse.json({error:"FULFILLMENT_NOT_FOUND"},{status:404});if(existing.status!==rule.to)return NextResponse.json({error:"INVALID_STATUS_TRANSITION",current:existing},{status:409});}
