...
export default function AvailabilityPage(){
  ...
  function authHeaders(json=false){const headers:Record<string,string>={};if(json)headers["Content-Type"]="application/json";if(token)headers.Authorization=`Bearer ${token}`;return headers}
  async function loadReservations(accessToken:string|null=token){if(!accessToken){setReservations([]);return}const response=await fetch("/api/v1/reservations",{headers:{Authorization:`Bearer ${accessToken}`},cache:"no-store"});if(response.ok)setReservations((await response.json()).reservations??[]);else setReservations([])}
  useEffect(()=>{let active=true;(async()=>{try{const config=await fetch("/api/v1/client-config").then(response=>response.json()),id=String(config.liffId??"");if(!active)return;setLiffId(id);if(!id)throw new Error("LINE認証を準備できませんでした");if(!window.liff)await new Promise<void>((ok,ng)=>{const s=document.createElement("script");s.src="https://static.line-scdn.net/liff/edge/2/sdk.js";s.onload=()=>ok();s.onerror=()=>ng();document.head.appendChild(s)});await window.liff!.init({liffId:id});if(!window.liff!.isLoggedIn()){window.liff!.login({redirectUri:location.href});return}const accessToken=window.liff!.getAccessToken();if(!accessToken)throw new Error("LINE認証を確認できませんでした");if(!active)return;setToken(accessToken);await loadReservations(accessToken)}catch(error){if(active){setReservations([]);setNotice(error instanceof Error?error.message:"予約情報を取得できませんでした")}}finally{if(active)setMemberReady(true)}})();return()=>{active=false}},[]);
  ...
  async function reserve(){
    ...
    if(!accessToken){setNotice("LINEでログインしてから予約してください");return}
