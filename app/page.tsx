...
function ServiceSheet({panel,member,onClose,onReservation}:{panel:Exclude<ServicePanel,null>;member:Member;onClose:()=>void;onReservation:()=>void}){
 ...
 return <div className="member-sheet-backdrop" onClick={onClose}><section className="member-sheet" role="dialog" aria-modal="true" aria-label={panel} onClick={event=>event.stopPropagation()}>
  ...
  {panel==="注文状況"&&((member.orders?.length??0)>0?<div className="member-sheet-content">{member.orders!.map(order=><div className="history-entry" key={order.orderNumber}><strong>{order.status==="WAITING_PAYMENT"?"お支払い待ち":order.status==="COOKING"?"ただいま調理中":order.status==="READY"?"商品ができあがりました":"注文受付済み"}</strong><p>{orderCallSummary(order)}</p><p>{scheduleText(order.schedule,order.scheduleLabel)}</p></div>)}</div>:<Empty text="進行中の注文はありません"/>)}
      ...
      {view === "member" && (
        <>
          ...
          <section className="activity-card">
              ...
              {activeOrders.map(order=><article className="activity-row" key={order.orderNumber}><span className="date-chip">ORDER</span><div><small>{orderCallSummary(order)||"注文受付済み"}</small><strong>{{ WAITING_PAYMENT: "お支払い待ち", ACCEPTED: "注文受付済み", COOKING: "ただいま調理中", READY: "商品ができあがりました" }[order.status]}</strong><p>{scheduleText(order.schedule,order.scheduleLabel)}</p></div><button onClick={() => openFutureFeature("注文状況")}>詳細</button></article>)}
