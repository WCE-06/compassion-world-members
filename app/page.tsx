...
export default function Home() {
  ...
  const installMembership=useCallback((next:Member)=>{setMember(next);next.notices.forEach(item=>seenNoticeIds.current.add(item.id))},[]);
  ...
  useEffect(()=>{if(view!=="member"||demo||!lineToken)return;let active=true;const sync=async()=>{try{const month=new Intl.DateTimeFormat("sv-SE",{timeZone:"Asia/Tokyo",year:"numeric",month:"2-digit"}).format(new Date()),response=await fetch(`/api/v1/me/points?month=${month}`,{headers:{Authorization:`Bearer ${lineToken}`},cache:"no-store"});if(!response.ok)return;const result=await response.json() as {balance:number};if(active&&Number.isFinite(result.balance))setMember(current=>({...current,points:Math.max(0,Math.trunc(result.balance))}))}catch{/* 会員証表示を妨げず次回同期で再試行する */}};void sync();const timer=window.setInterval(sync,30_000);const onVisible=()=>{if(document.visibilityState==="visible")void sync()};document.addEventListener("visibilitychange",onVisible);return()=>{active=false;window.clearInterval(timer);document.removeEventListener("visibilitychange",onVisible)}},[view,demo,lineToken]);
