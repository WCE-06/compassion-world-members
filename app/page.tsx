...
export default function Home() {
  ...
  const register=async()=>{if(registering)return;setRegistering(true);setNotice("");setRegistrationStage("登録情報を安全に保存しています");try{if(demo){await new Promise(resolve=>setTimeout(resolve,500));const code="N8W4K2M7P5";setRegistrationStage("会員番号を発行しています");await new Promise(resolve=>setTimeout(resolve,350));setMember({...DEMO_MEMBER,memberCode:code,displayName:registration.displayName||DEMO_MEMBER.displayName,smaregiSyncStatus:"PENDING"});setView("member");setSelectedNotice({id:"welcome-new-demo",category:"NEWS",title:"会員登録ありがとうございます",body:`${registration.displayName||"お客様"} 様\n\nCOMPASSION WORLDのポイントカードへようこそ。\n会員番号は ${code} です。\n\nポイント、スタジオ予約、モバイルオーダー、会員限定特典をこの画面からご利用いただけます。`,createdAt:"ただいま",unread:true});return}const response=await fetch("/api/v1/members",{method:"POST",headers:{Authorization:`Bearer ${lineToken}`,"Content-Type":"application/json"},body:JSON.stringify(registration)}),result=await response.json();if(!response.ok)throw new Error(result.message??(result.error==="PHONE_ALREADY_REGISTERED"?"同じ電話番号の会員情報があります。以前の会員番号から引き継いでください":"登録内容を確認してください"));setRegistrationStage("会員番号を発行しています");const membership=await fetch("/api/v1/me/membership",{headers:{Authorization:`Bearer ${lineToken}`}});if(!membership.ok)throw new Error("会員証を取得できませんでした。もう一度お試しください");setRegistrationStage("会員証を準備しています");const registered=await membership.clone().json();setMember(await membership.json());setView("member");setSelectedNotice(registered.notices?.find((item:MemberNotice)=>item.id.startsWith("welcome:"))??null)}catch(error){setNotice(error instanceof Error?error.message:"登録できませんでした");setView("confirm")}finally{setRegistrationStage("");setRegistering(false)}};
  const launchReservation=async()=>{if(openingReservation)return;if(!lineToken){setNotice("LINE認証を確認できませんでした。会員証を開き直してください");return}setOpeningReservation(true);setNotice("FEBBRAIO予約サイトへ接続しています…");try{const response=await fetch("/api/v1/febbraio/launch",{method:"POST",headers:{Authorization:`Bearer ${lineToken}`}}),result=await response.json();if(!response.ok)throw new Error(result.message??"予約サイトへ接続できませんでした");const form=document.createElement("form");form.method="POST";form.action=result.exchangeUrl;const input=document.createElement("input");input.type="hidden";input.name="token";input.value=result.token;form.appendChild(input);document.body.appendChild(form);form.submit()}catch(error){setNotice(error instanceof Error?error.message:"予約サイトへ接続できませんでした");setOpeningReservation(false)}};
